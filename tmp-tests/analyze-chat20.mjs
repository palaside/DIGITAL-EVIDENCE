...
const outputDir = String.raw`D:\Project\หลักฐานดิจิทัล  DIGITAL EVIDENCE\tmp-tests\chat20-analysis`;
...
await page.waitForFunction(
  () => {
    const pageImages = document.querySelectorAll('img[alt^="A4 Page"]');
...
const pageCards = page.locator('img[alt^="A4 Page"]').locator("..");
const cardCount = await page.locator('img[alt^="A4 Page"]').count();
...
await browser.close();
console.log(JSON.stringify({ outputDir, sourceCount: files.length, pageCount: summary.length }, null, 2));
